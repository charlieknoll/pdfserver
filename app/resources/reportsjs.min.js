...
function Page(element, report, isConsolePage) {
    var self = this;
    self.element = element;
...
function Report(target, source, opt) {
    ...
    source.classList.add(self.options.paperType());
...
Report.prototype.init = function (target, source) {
    ...
    self.removeAttributes(sourceCopyEl, 'contenteditable');
    ...
    self.options.groupNodeNames.map(function (elName) {
        return self.addClassToElements(sourceCopyEl, elName, self.options.groupClass);
    });
    self.options.groupHeaderNodeNames.map(function (elName) {
        return self.addClassToElements(sourceCopyEl, elName, self.options.groupHeaderClass);
    });
    self.headers = sourceCopy.getElementsByClassName(self.options.pageHeaderClass);
    self.footers = sourceCopy.getElementsByClassName(self.options.pageFooterClass);
    self.setPageInnerHeight(sourceCopy);
    self.pages = [];
    self.lastPage = new Page(sourceCopy, self);
...
Report.prototype.addConsolePage = function () {
    ...
    div.innerHTML = "<ul class='rp-logs'>"
    rp.logs.forEach(function (l) {
        div.innerHTML += "<li>" + l + "</li>"
    })
    div.innerHTML += "</ul>"
...
Report.prototype.insertPage = function () {
    var self = this;
    var newPage = new Page(self.lastPage.element.cloneNode(false), self);
    self.pages.splice(self.pages.length - 1, 0, newPage);
    self.lastPage.element.parentNode.insertBefore(newPage.element, self.lastPage.element);
    var els, j;
...
Report.prototype.splitPages = function () {
    ...
    if (self.moveChildElements(self.lastPage.innerContent, newPage, newPage.innerContent, newPage.firstFooter)) {
        ...
        if (newPage.pageBreak) self.splitPages()
        self.lastPage.setAvailableHeight();
