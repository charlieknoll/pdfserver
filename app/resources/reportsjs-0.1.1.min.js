...
function Group(element, options, index) {
  var self = this;
  self.element = element;
  self.element.dataset.rpGroupIndex = index
  ...
  for (var i = 0; i < headerElements.length; i++) {
    ...
    if (self.element === headerElements[i].parentElement) {
      ...
      if (!headerElements[i].parentElement.classList.contains(options.groupClass)) {
...
Group.prototype.showHiddenOnFirstPage = function (targetPage, parentElement) {

  var self = this
  if (targetPage === self.firstPage) return
  var headerElements = parentElement.getElementsByClassName(self.options.groupHeaderClass)

  for (var i = 0; i < headerElements.length; i++) {
    if (headerElements[i].parentElement.dataset.rpGroupIndex === self.element.dataset.rpGroupIndex) {
      //remove class hideOnFirstPage of any of the group's headers
      var els = headerElements[i].getElementsByClassName(self.options.hideOnFirstPageClass);
...
Report.prototype.upsertGroup = function (element, targetPage) {
    ...
    if (!group) {
        group = new Group(element, self.options, self.groups.length);
...
Report.prototype.moveChildElements = function (source, targetPage, targetParent, targetReference, targetGroup) { //returns true if page is full, false if more space
    ...
    for (var j = 0; j < childNodes.length; j++) {
        ...
        if (self.isGroupBand(elementToMove) && !isKeepTogetherNode) {
            ...
            self.groups.map(g => g.showHiddenOnFirstPage(targetPage, elShadow))
...
(function init() {
    ...
    console.log = function (msg) {
        rp.logs.push((new Date).toISOString().substring(11, 23) + " : " + msg)
